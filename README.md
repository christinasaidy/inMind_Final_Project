...
In my nodes you can also see different methods i tried for tool integration. The store agent, manually invokes each tool call made by the model. It loops over each tool name and manually adds a ToolMessage. In the rag node, i explored langgraphs built in *ToolNode* that also invokes the tools using a loop, it defines ToolMessages for you . In the query agent, i tried the built in create react agent that automatatically does tool calls without doing anything manually. All three methods work the same and effectively.
